...
from pr2.connection import init_db, get_session
from pr2.models import (
...
def profession_update(
    profession_id: int, prof: ProfessionDefault, session=Depends(get_session)
) -> ProfessionDefault:
...
# --- Warrior-Skill link endpoint ---

@app.post("/warrior/{warrior_id}/skill/{skill_id}")
def warrior_add_skill(warrior_id: int, skill_id: int, session=Depends(get_session)):
    warrior = session.get(Warrior, warrior_id)
    if not warrior:
        raise HTTPException(status_code=404, detail="Warrior not found")
    skill = session.get(Skill, skill_id)
